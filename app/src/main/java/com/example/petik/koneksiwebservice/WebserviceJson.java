...
public class WebserviceJson extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.layoutjson);
    ...
    private void uraiJSON(){
       String url = "https://api.myjson.com/bins/opcfg";

        JsonObjectRequest request = new JsonObjectRequest (Request.Method.GET, url, null, new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject response) {
                try {
                    JSONArray jsonArray = response.getJSONArray("mahasantri");
                    for (int i = 0; i < jsonArray.length(); i++) {
                        ...
                        String asal_daerah = mahasantri.getString("asal_daerah");
                        String kamar = mahasantri.getString("kamar");

                        textHasilJSON.append(id+", " +nama+ ", " + asal_daerah + ", "
                        +kamar +"\n\n");
                        }
